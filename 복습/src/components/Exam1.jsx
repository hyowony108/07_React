...
class Exam1 extends Component {
  ...
  // 컴포넌트가 처음 화면에 나타났을 때
  componentDidMount() {
    console.log("componentDidMount : 마운트 확인 완료");
  }

  // props나 state가 변경된 후
  componentDidUpdate(prevProps, prebState) {
    console.log("componentDidUpdate : 업데이트 완료");
    console.log("이전 state : ", prebState.count);
  ...
  render() {
    return (
      <div>
        ...
        <h2>
          부모로 부터 전달받은 값 : {this.props.hyowon} {this.props.test}
        </h2>
      </div>
    );
  }
...
// 다른 파일에서 해당 파일(Exam1.jsx)의 기본으로 지정된 컴포넌트(Exam1)를 사용할 수 있도록 내보냄(수출함)
